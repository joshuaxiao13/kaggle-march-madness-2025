...
for (year in all_years) {
  csv_file <- read.csv(paste0("//wsl.localhost/Ubuntu-24.04/home/joshux/projects/march-madness-2025/datasets/Xy_OLRE_W_", year, ".csv"))
...
train_model <- function(year, all_years, Xy_OLRE) {
  ...
  model <- MASS::polr(ordered(Tournament.Wins, levels=0:6) ~ WinPercentage + PointDIFF + WAB + Top30Wins, data=Xy_train)
  print(summary(model))
  model
}
...
train_and_save_estimates_csv_by_year <- function(year, all_years, Xy_OLRE, return_mse = TRUE) {
  ...
  write.csv(expected_wins_df[[as.character(year)]], paste0("//wsl.localhost/Ubuntu-24.04/home/joshux/projects/march-madness-2025/prediction/expected-tournament-wins-W-", year, ".csv"), row.names=FALSE)
  if (return_mse) {
...
# Predict 2025
Xy_OLRE$`2025` <- read.csv(paste0("//wsl.localhost/Ubuntu-24.04/home/joshux/projects/march-madness-2025/datasets/Xy_OLRE_W_2025.csv"))
